...
def load_data():
    ...
    graphs = pd.read_csv(r'./hist_data/graphs.csv')
    progress.progress(100)
    
    status_text.text("Loading historical data... --- Progress : 100% --- All done !")
    
    return bureau_agg, prev_agg, pos_agg, ins_agg, cc_agg, graphs
...
bureau_agg, prev_agg, pos_agg, ins_agg, cc_agg, graphs = load_data()
    ...
    fig = go.Figure(go.Indicator(
        ...
        gauge={
            ...
            'steps': [
                ...
                {'range': [10, optimal_threshold*100], 'color': "orange"},
                {'range': [optimal_threshold*100, 30], 'color': "red"}
            ],
...
def create_plots(column_name, vertical_line_value):
    sns.set(rc={'axes.facecolor':'none', 'figure.facecolor':'none'})
    plt.figure(figsize=(4, 2),facecolor='none')
    sns.kdeplot(graphs[column_name], fill=True, color="blue")
    plt.axvline(x=vertical_line_value, color='red', linestyle='--', linewidth=1.5)
    plt.xlabel('')
    plt.ylabel('')  
...
def main():
    ...
    if 'data' in st.session_state:
        ...
        with col1:
            ...
            plot1 = create_plots('AMT_INCOME_TOTAL', amt_income_total)
            st.pyplot(plot1)
            st.markdown(' ')

            st.markdown('##### Price of Goods')
            plot3 = create_plots('AMT_GOODS_PRICE', amt_goods_price)
            st.pyplot(plot3)



        with col2:
            st.markdown('##### Amount of Credit Asked')
            plot2 = create_plots('AMT_CREDIT', amt_credit)
            st.pyplot(plot2)
            st.markdown(' ')

            st.markdown('##### Loan annuity')
            plot4 = create_plots('AMT_ANNUITY', amt_annuity)
            st.pyplot(plot4)
